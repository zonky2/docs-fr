...
final readonly class MetaModelsServiceExamplesListener
{
    ...
    public function onMetaModelsServiceExamples(PrePersistModelEvent $event): void
    {
        ...
        // Security - plus d'explications dans le paragraphe suivant.
        // Recommandé : security.helper - permet de vérifier les droits ET de récupérer l'utilisateur.
        ...
        // Recharger le MemberModel Contao (avec tous les champs de la base de données) via le nom d'utilisateur.
        $member = MemberModel::findByUsername($user->getUserIdentifier());

        // Alternative 1 : Contao-Framework (Legacy). getInstance() renvoie l'utilisateur
        // frontend actuel sous forme d'objet Contao, avec tous les champs de la base de
        // données - createInstance() créerait une nouvelle instance vide et serait donc incorrect.
        $member = $this->framework->getAdapter(FrontendUser::class)->getInstance();

        // Alternative 2 : security.token_storage (bas niveau, sans vérification des droits).
